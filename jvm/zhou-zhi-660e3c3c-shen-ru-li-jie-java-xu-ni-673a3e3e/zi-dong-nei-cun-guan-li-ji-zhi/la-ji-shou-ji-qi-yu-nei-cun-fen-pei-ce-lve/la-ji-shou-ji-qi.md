...
**初始标记** 初始标记仅仅只是标记一下GC Roots能直接关联到的对象，速度很快，需要**Stop The World**\(暂停所有的用户线程\)

**并发标记** 并发标记阶段就是进行GC Roots Tracing的过程** \(用户不暂停\)**—用户不暂停就还可能产生一些对象与GC Roots不可达

**重新标记 **重新标记阶段是为了**修正 并发标记期间 因用户程序继续运作而导致标记产生变动** 的那一部分对象的标记记录，这个阶段的停顿时间会比初始阶段稍长一些,但是远比并发标记的时间短，仍然需要"**Stop The World**"
...
你想追求低停顿、想让用户有更好的体验用G1如果你的应用追求吞吐量，G1并不能带来很明显的好处。
