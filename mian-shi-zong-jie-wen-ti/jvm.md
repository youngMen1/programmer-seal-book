...
看[10、JVM内存结构，为什么需要GC?](https://blog.yk95.top/2018/01/30/成为Java顶尖程序员，先过了下面问题！（答案）/#10、JVM内存结构，为什么需要GC？)

## 8、常用的GC策略，什么时候会触发YGC，什么时候触发FGC？ {#8、常用的GC策略，什么时候会触发YGC，什么时候触发FGC？}
...
对YGC的 触发时机，相当的显而易见，就是Eden区空间不足， 这时候就肯定会触发YGC
