...
![](https://img-blog.csdn.net/20180115171334134)
...
### 公钥被调包了怎么办？又是一个鸡生蛋蛋生鸡问题？ {#公钥被调包了怎么办又是一个鸡生蛋蛋生鸡问题}

但是方案1有个问题：如果服务器端发送公钥给客户端时，被中间人调包了，怎么办？

我画了张图方便理解：

![](https://img-blog.csdn.net/20180115174255906)

显然，让每个客户端的每个浏览器默认保存所有网站的公钥是不现实的。

### 使用第三方机构的公钥解决鸡生蛋蛋生鸡问题 {#使用第三方机构的公钥解决鸡生蛋蛋生鸡问题}
...
![](https://img-blog.csdn.net/20180115174335511)

如果能解密，就说明这个公钥没有被中间人调包。因为如果中间人使用自己的私钥加密后的东西传给客户端，客户端是无法使用第三方的公钥进行解密的。

![](https://img-blog.csdn.net/20180115174402845)
...
![](https://img-blog.csdn.net/20180115174434026)

客户端能解密同一家第三机构颁发的所有证书：

![](https://img-blog.csdn.net/20180115174500531)

最终导致其它持有同一家第三方机构证书的中间人可以进行调包：

![](https://img-blog.csdn.net/20180115174531489)

### 数字签名，解决同一机构颁发的不同证书被篡改问题 {#数字签名解决同一机构颁发的不同证书被篡改问题}
...
![](https://img-blog.csdn.net/20180115174639441)

当客户端拿到证书后，开始对证书中的内容进行验证，如果客户端计算出来的证书编号与证书中的证书编号相同，则验证通过：

![](https://img-blog.csdn.net/20180115174713275)
...
### CA如何颁发数字证书给服务器端的？ {#ca如何颁发数字证书给服务器端的}
...
![](https://img-blog.csdn.net/20180115174740186)

拿到证书后，我们就可以将证书配置到自己的服务器上了。那么如何配置？这是具体细节了，留给大家google了。

### 也许我们需要整理一下思路 {#也许我们需要整理一下思路}
...
![](https://img-blog.csdn.net/20180115174809938)

### 能不能用一句话总结HTTPS？ {#能不能用一句话总结https}

**1.使用非对称加密协商加密算法**

**2.使用对称加密方式传输数据**

**3.使用第三方机构签发的证书，来加密公钥，用于公钥的安全传输、防止被中间人串改。**
